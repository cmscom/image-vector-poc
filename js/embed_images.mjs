/**
 * Generate SigLIP image embeddings using Transformers.js (ONNX Runtime).
 *
 * Usage:
 *   node embed_images.mjs --image-list <json_path> --output <json_path> [--dtype fp32]
 *
...
if (!args['image-list'] || !args['output']) {
  console.error('Usage: node embed_images.mjs --image-list <json> --output <json> [--dtype fp32|fp16|q8|q4]');
  process.exit(1);
}

/**
 * L2-normalize an embedding array (matches Python implementation).
 * Python: embeddings / np.clip(np.linalg.norm(embeddings, axis=-1, keepdims=True), 1e-8, None)
 */
